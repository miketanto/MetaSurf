...
def load_definitions(
    conn: psycopg.Connection, exclude_archetype_files: frozenset[str] = frozenset()
...
def load_decks(
    ...
    """All Modern decks with event date in [start, end], ordered by deck id.
    Zones come from deck_cards (board 'main'/'side'); empty decks excluded."""
    with conn.cursor() as cur:
        cur.execute(
            """
